...
export class AuthService {
  ...
  public async logout(dto: ILogoutDto): Promise<void> {
    ...
    await this.prismaService.auth.deleteMany({ where: { refreshToken } });
  }

  public async refresh(dto: IRefreshDto): Promise<IJwt> {
    const { refreshToken } = dto;
    const authEntity = await this.prismaService.auth.findFirst({
      where: { refreshToken },
      include: { user: true },
    });

    if (!authEntity || authEntity.refreshTokenExpiresAt < BigInt(new Date().getTime())) {
  ...
  public async signup(data: IUserCreateDto): Promise<User> {
